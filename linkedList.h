...
namespace LinkedList {
	class Node {
	private:
		const string value;    // The data held by the LinkedList
		unique_ptr<Node> next; // unique_ptr to the next node
		Node* prev;                 // raw (non-owning) ptr to the previous node
	public: 
		
		// 2 constructors  - why always one empty constructor ?
		...
		Node(const string& value, unique_ptr<Node> next, Node* prev)
			: value(value)
			, next(std::move(next))
			, prev(prev)
		{}
	...
	public:
		...
		Node* insert(Node *pos, const string& value);
